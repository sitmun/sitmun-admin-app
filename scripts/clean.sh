...
show_help() {
  ...
  echo ""
  echo "Comprehensive cleaning script for admin-app Angular project"
  echo ""
  echo "Options:"
...
validate_project_root() {
  ...
  if ! grep -q '"name"[[:space:]]*:[[:space:]]*"admin-app"' package.json; then
    echo -e "${RED}ERROR: Not the admin-app project${NC}"
    echo "Found package.json but it's not for admin-app"
    exit 1
  fi
...
echo ""
echo -e "${BLUE}═══════════════════════════════════════${NC}"
echo -e "${BLUE}  Cleaning Admin App${NC}"
echo -e "${BLUE}═══════════════════════════════════════${NC}"
echo ""
...
remove_item "dist/admin-app" "Angular build output"
